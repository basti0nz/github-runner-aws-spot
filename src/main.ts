...
async function startRunner(token: string, params: IEC2Params): Promise<string> {
  ...
  const githubRegistrationToken = await ghc.getRegistrationToken();
  const aws = new awsClient(params)
  const ec2InstanceId = await aws.startEc2Instance();
  await aws.waitForInstanceRunning(ec2InstanceId);
  await ghc.waitForRunnerRegistered(params.label!);
  return ec2InstanceId
}

async function stopRunner(token: string, label: string, ec2InstanceId: string) {
  let params: IEC2Params = {
...
async function run(): Promise<void> {
  try {
    ...
    if (mode === 'start') {
      ...
      if (!params.ec2ImageId || !params.ec2InstanceType || !params.subnetId || !params.securityGroupId) {
