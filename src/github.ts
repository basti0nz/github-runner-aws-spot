...
export class gitHubClient implements GitHubWorker {
	...
	async getRegistrationToken(): Promise<any> {
		try {
			const response = await this.octokit.request('POST /repos/{owner}/{repo}/actions/runners/registration-token', this.githubContext);
	...
	async getRunner(): Promise<null | any> {
		try {
			const runners = await this.octokit.paginate('GET /repos/{owner}/{repo}/actions/runners', this.githubContext);
	...
	async removeRunner(): Promise<void> {
		...
		if (!runner) {
			core.info(`GitHub self-hosted runner with label ${this.label} is not found, so the removal is skipped`);
			return;
		}
		try {
			await this.octokit.request('DELETE /repos/{owner}/{repo}/actions/runners/{runner_id}', _.merge(this.githubContext, { runner_id: runner.id }));
